...
def process(set_='test', annotation_filename='raw/annotation_val.odgt',
    ...
    # write the 'crowdhuman/train.txt' or 'crowdhuman/test.txt'
...
def main():
    ...
    rm_txts(output_dir)
    process('test', 'crowdhuman-%s/raw/annotation_val.odgt' % args.dim, output_dir)
    process('train', 'crowdhuman-%s/raw/annotation_train.odgt' % args.dim, output_dir)

    with open('crowdhuman-%s.data' % args.dim, 'w') as f:
        f.write("""classes = 2
train   = crowdhuman-%s/train.txt
valid   = crowdhuman-%s/test.txt
names   = crowdhuman.names
backup  = backup/\n""" % (args.dim, args.dim))
